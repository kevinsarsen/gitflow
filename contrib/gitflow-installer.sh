...
if [ -z "$REPO_HOME" ] ; then
	REPO_HOME="https://github.com/kevinsarsen/gitflow.git"
fi
...
EXEC_FILES="git-flow"
SCRIPT_FILES="git-flow-init git-flow-feature git-flow-hotfix git-flow-bugfix git-flow-release git-flow-support git-flow-version gitflow-common gitflow-shFlags git-flow-config"
